...
#define TAG_C "ServiceCheat"
#define LOGV(...) __android_log_print(ANDROID_LOG_VERBOSE, TAG_C, __VA_ARGS__)
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG,   TAG_C, __VA_ARGS__)
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO,    TAG_C, __VA_ARGS__)
#define LOGW(...) __android_log_print(ANDROID_LOG_WARN,    TAG_C, __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR,   TAG_C, __VA_ARGS__)

extern std::string sysprop(const char* name, const char* def = "");
extern bool file_exists(const char* path);
extern std::string read_file(const char* path, size_t maxBytes = 8192);
extern std::vector<std::string> read_lines(const char* path);
extern bool dir_exists(const char* path);
extern bool port_open(const char* host, int port, int timeout_ms);
extern long long mono_ms();
extern std::vector<pid_t> find_procs_by_name(const char* name);
extern std::string read_proc_cmdline(pid_t pid);
extern std::string primary_abi();
extern int sdk_int();
extern bool is_debuggable();
extern std::string j2s(JNIEnv* e, jstring j);
extern jstring s2j(JNIEnv* e, const std::string& s);
extern std::atomic<int> g_threat_level;
...
static const char* ROOT_BINARIES[] = {
    ...
    "/system/xbin/mu",          "/system/app/Magisk.apk",
    "/data/adb/magisk",         "/data/adb/ksud",
    "/system/lib/libsupol.so",
    nullptr
};

static bool check_root_binaries() {
    for (int i = 0; ROOT_BINARIES[i]; ++i) {
        if (file_exists(ROOT_BINARIES[i])) {
            LOGW("Root binary: %s", ROOT_BINARIES[i]);
            return true;
        }
    }
    return false;
...
static bool check_root_rw_system() {
    FILE* f = fopen("/proc/mounts", "r");
    if (!f) return false;
    char line[1024];
    bool found = false;
    while (fgets(line, sizeof(line), f)) {
        if (strstr(line, " /system ") && strstr(line, "rw")) { found = true; break; }
    }
    fclose(f);
    return found;
}

static bool check_root_props() {
    auto build_tags = sysprop("ro.build.tags");
    if (build_tags.find("test-keys") != std::string::npos) return true;
    auto secure = sysprop("ro.secure");
    if (secure == "0") return true;
    return false;
}

static bool check_root_su_access() {
    return !find_procs_by_name("su").empty();
}

static bool check_root_magisk() {
    const char* magisk_paths[] = {
        "/data/adb/magisk", "/data/adb/magisk.db",
        "/sbin/.magisk",    "/dev/.magisk.unblock",
        "/cache/.disable_magisk", "/data/adb/modules",
        nullptr
    };
    for (int i = 0; magisk_paths[i]; ++i) {
        if (file_exists(magisk_paths[i])) { LOGW("Magisk: %s", magisk_paths[i]); return true; }
    }
    return false;
}

static bool check_root_kernelsu() {
    const char* ksu_paths[] = { "/data/adb/ksud", "/data/adb/ksu", "/sbin/ksud", nullptr };
    for (int i = 0; ksu_paths[i]; ++i) {
        if (file_exists(ksu_paths[i])) { LOGW("KernelSU: %s", ksu_paths[i]); return true; }
    }
    return false;
}
...
bool detect_root() {
    return check_root_binaries()  ||
           check_root_rw_system() ||
           check_root_props()     ||
           check_root_magisk()    ||
           check_root_kernelsu()  ||
           check_root_su_access();
}

static bool check_emu_props() {
    struct { const char* key; const char* val; } emu_props[] = {
    ...
    for (int i = 0; emu_props[i].key; ++i) {
        auto val = sysprop(emu_props[i].key);
        if (val == emu_props[i].val || (val.find(emu_props[i].val) != std::string::npos && strlen(emu_props[i].val) > 3)) {
            LOGW("Emulator prop: %s = %s", emu_props[i].key, val.c_str());
            return true;
        }
    }
    return false;
...
static bool check_emu_files() {
    const char* emu_files[] = {
        "/dev/socket/qemud",  "/dev/qemu_pipe",
        "/system/lib/libc_malloc_debug_qemu.so",
        "/sys/qemu_trace",    "/system/bin/qemu-props",
        "/dev/goldfish_pipe", "/dev/socket/genyd",
        nullptr
    };
    for (int i = 0; emu_files[i]; ++i) {
        if (file_exists(emu_files[i])) { LOGW("Emulator file: %s", emu_files[i]); return true; }
    }
    return false;
}

static bool check_emu_net() {
    auto ip = read_file("/proc/net/fib_trie");
    return ip.find("10.0.2.2") != std::string::npos;
}

static bool check_emu_sensors() {
    auto content = read_file("/proc/self/maps");
    return content.find("libemulator") != std::string::npos ||
           content.find("libgoldfish") != std::string::npos;
}

bool detect_emulator() {
    return check_emu_props() || check_emu_files() || check_emu_net() || check_emu_sensors();
}

static bool check_tracerpid() {
...
static bool check_ptrace_self() {
#if defined(__aarch64__) || defined(__x86_64__)
    if (ptrace(PTRACE_TRACEME, 0, nullptr, nullptr) == -1) {
        LOGW("ptrace TRACEME failed: %s", strerror(errno));
        return true;
    }
...
static bool check_android_debugger() {
    auto props = sysprop("ro.debuggable");
    if (props == "1" && sysprop("init.svc.adbd") == "running") return true;
    return false;
}

static bool check_frida_server() {
    ...
    for (int i = 0; frida_paths[i]; ++i) {
        if (file_exists(frida_paths[i])) { LOGW("Frida server: %s", frida_paths[i]); return true; }
    }
...
static bool check_frida_maps() {
    ...
    const char* frida_libs[] = {
        ...
        "libfrida", "frida-agent", "re.frida",
        nullptr
    };
    for (int i = 0; frida_libs[i]; ++i) {
        if (maps.find(frida_libs[i]) != std::string::npos) {
            LOGW("Frida maps: %s", frida_libs[i]);
            return true;
        }
    }
    return false;
...
static bool check_frida_ports() {
    for (int port : {27042, 27043, 27044, 27045}) {
        if (port_open("127.0.0.1", port, 200)) { LOGW("Frida port open: %d", port); return true; }
    }
    return false;
}

static bool check_xposed_files() {
    ...
    for (int i = 0; xposed_files[i]; ++i) {
        if (file_exists(xposed_files[i])) { LOGW("Xposed file: %s", xposed_files[i]); return true; }
    }
    return false;
}

static bool check_lsposed_files() {
    ...
    for (int i = 0; lsp_paths[i]; ++i) {
        if (file_exists(lsp_paths[i])) { LOGW("LSPosed: %s", lsp_paths[i]); return true; }
    }
    return !find_procs_by_name("lspd").empty();
}

static bool check_hook_inline() {
    void* handle = dlopen("libc.so", RTLD_NOW);
    if (!handle) return false;
    void* sym = dlsym(handle, "open");
    dlclose(handle);
    if (!sym) return false;
    uint8_t* ptr = reinterpret_cast<uint8_t*>(sym);
...
static bool check_gameguardian() {
    const char* gg_paths[] = {
        "/data/data/catch_.me_.if_.you_.can_",
        "/data/data/com.cih.gamecih",
        "/data/data/com.cih.gamecih2",
        "/data/app/com.niemoney.my.cheatengine",
        nullptr
    };
    for (int i = 0; gg_paths[i]; ++i) {
        if (file_exists(gg_paths[i])) { LOGW("GameGuardian/CheatEngine: %s", gg_paths[i]); return true; }
    }

    const char* gg_procs[] = { "gameguardian", "gg.service", "GameGuardian", nullptr };
    for (int i = 0; gg_procs[i]; ++i) {
        if (!find_procs_by_name(gg_procs[i]).empty()) {
            LOGW("GameGuardian process: %s", gg_procs[i]);
            return true;
        }
    }
    return false;
...
static bool check_test_keys() {
    auto tags = sysprop("ro.build.tags");
    return tags.find("test-keys") != std::string::npos || tags.find("dev-keys") != std::string::npos;
}

static bool check_build_props_modified() {
    auto build_type = sysprop("ro.build.type");
    return build_type == "eng" || build_type == "userdebug";
}

static bool check_binary_modified() {
    auto self_maps = read_file("/proc/self/maps");
    size_t rw_count = 0;
    std::istringstream ss(self_maps);
    std::string line;
    while (std::getline(ss, line)) {
    ...
    if (rw_count > 10) { LOGW("Suspicious rw segments: %zu", rw_count); return true; }
    return false;
}

bool detect_tamper() {
    return check_test_keys() || check_build_props_modified() || check_binary_modified();
}

static bool check_native_lib_integrity() {
    auto maps = read_file("/proc/self/maps");
    size_t lib_count = 0;
    std::istringstream ss(maps);
    std::string line;
    while (std::getline(ss, line)) {
        if (line.find("libgamecore.so") != std::string::npos) ++lib_count;
    }
    return lib_count > 0;
}

static bool check_dex_integrity() {
    return dir_exists("/data/dalvik-cache");
}

bool detect_integrity(const char* pkg) {
    (void)pkg;
    return check_native_lib_integrity() && check_dex_integrity();
}
...
static std::vector<MemRegion> parse_maps() {
    ...
    auto lines = read_lines("/proc/self/maps");
    for (auto& line : lines) {
        MemRegion r{};
...
static bool detect_frida_gadget_in_memory() {
    auto regions = parse_maps();
    for (auto& r : regions) {
        if (r.name.find("frida") != std::string::npos) return true;
        if (r.name.find("gadget") != std::string::npos) return true;
    }
    return false;
}

static size_t count_mapped_libraries() {
    auto regions = parse_maps();
    std::set<std::string> libs;
    for (auto& r : regions) {
        if (r.name.find(".so") != std::string::npos) libs.insert(r.name);
    }
    return libs.size();
}
...
static bool detect_suspicious_libs() {
    auto regions = parse_maps();
    const char* suspicious[] = { "frida","inject","xposed","lspd","substrate","cydia","hook","patch","cheat", nullptr };
    for (auto& r : regions) {
        std::string nl = r.name;
        std::transform(nl.begin(), nl.end(), nl.begin(), ::tolower);
        for (int i = 0; suspicious[i]; ++i) {
            if (nl.find(suspicious[i]) != std::string::npos) {
                LOGW("Şüpheli kütüphane: %s", r.name.c_str());
                return true;
            }
        }
    }
...
static ProcInfo get_proc_info(pid_t pid) {
    ...
    auto status  = read_file((base + "/status").c_str());
    std::istringstream ss(status);
    std::string line;
    while (std::getline(ss, line)) {
        if (line.rfind("Name:", 0) == 0)  info.name    = line.substr(5);
        if (line.rfind("State:", 0) == 0) info.state   = line.substr(6);
        if (line.rfind("Uid:", 0) == 0)   sscanf(line.c_str()+4, "%d", &info.uid);
        if (line.rfind("VmRSS:", 0) == 0) sscanf(line.c_str()+6, "%ld", &info.vmrss_kb);
    }
    while (!info.name.empty() && (info.name.back() == '\n' || info.name.front() == '\t' || info.name.front() == ' ')) {
        if (info.name.front() == '\t' || info.name.front() == ' ') info.name.erase(0,1);
        else info.name.pop_back();
    }
...
static bool detect_suspicious_processes() {
    const char* bad_procs[] = {
        "frida","gdbserver","strace","ltrace",
        "tcpdump","burp","objection","r2","radare","jdwp",
        nullptr
    };
    auto procs = scan_all_processes();
    for (auto& p : procs) {
        std::string nl = p.name;
        std::transform(nl.begin(), nl.end(), nl.begin(), ::tolower);
        for (int i = 0; bad_procs[i]; ++i) {
            if (nl.find(bad_procs[i]) != std::string::npos) {
                LOGW("Şüpheli process: %s (pid=%d)", p.name.c_str(), p.pid);
                return true;
            }
        }
    }
...
static bool detect_mitm_proxy() {
    for (const char* file : {"/proc/net/tcp", "/proc/net/tcp6"}) {
        ...
        if (content.find("5C11") != std::string::npos ||
            content.find("1F90") != std::string::npos ||
            content.find("BB01") != std::string::npos) {
            LOGW("MITM proxy indicator in %s", file);
            return true;
        }
    }
    return false;
...
static bool check_tee_available() {
    return file_exists("/dev/trusty-ipc-dev0") || file_exists("/dev/tee0") || file_exists("/dev/teepriv0");
}

static bool check_verified_boot() {
    auto vb_state = sysprop("ro.boot.verifiedbootstate");
    if (vb_state == "green" || vb_state == "yellow") return true;
    if (vb_state == "orange") { LOGW("Verified boot: orange (unlocked)"); return false; }
    if (vb_state == "red")    { LOGW("Verified boot: red (compromised)"); return false; }
    return sysprop("ro.boot.veritymode") == "enforcing";
}

static bool check_encryption_status() {
    return sysprop("ro.crypto.state") == "encrypted";
}

static bool check_art_integrity() {
    auto maps = read_file("/proc/self/maps");
    return maps.find("libart.so") != std::string::npos &&
           (maps.find("core.vdex") != std::string::npos || maps.find("boot.art") != std::string::npos);
}
...
static bool check_zygote_parent() {
    ...
    while (std::getline(ss, line)) {
        if (line.rfind("PPid:", 0) == 0) {
            pid_t ppid = 0;
            sscanf(line.c_str()+5, "%d", &ppid);
            if (ppid > 0) {
                auto parent_name = get_proc_info(ppid).name;
                return parent_name.find("zygote") != std::string::npos;
            }
        }
    }
    return false;
...
static bool detect_timing_attack() {
    auto t1 = mono_ms();
    volatile int sum = 0;
    for (int i = 0; i < 1000000; ++i) sum += i;
    auto t2 = mono_ms();
    if (t2 - t1 > 500) { LOGW("Zamanlama anomalisi: %lld ms", t2 - t1); return true; }
    return false;
}

static bool detect_vm_acceleration_missing() {
    auto renderer = sysprop("ro.hardware.egl");
    return renderer == "swiftshader" || renderer == "mesa" ||
           sysprop("ro.opengles.renderer").find("SwiftShader") != std::string::npos;
}

static bool detect_adb_always_on() {
    return sysprop("init.svc.adbd") == "running" && sysprop("ro.secure") != "1";
}

bool protect_self() {
    if (prctl(PR_SET_DUMPABLE, 0, 0, 0, 0) != 0) LOGW("prctl PR_SET_DUMPABLE başarısız");
    if (prctl(PR_SET_NAME, "eaquel_worker", 0, 0, 0) != 0) LOGW("prctl PR_SET_NAME başarısız");
    return true;
}

void enable_secure_mode() {
    protect_self();
...
public:
    ...
    bool start(const std::vector<std::string>& paths) {
        ...
        for (auto& p : paths) {
            int wd = inotify_add_watch(fd_, p.c_str(),
...
private:
    ...
    std::thread thread_;
    std::mutex mutex_;
    std::map<int,std::string> watch_map_;
        ...
        while (running_.load()) {
            ...
            while (pos < static_cast<size_t>(n)) {
                ...
                if (ev->wd >= 0 && watch_map_.count(ev->wd)) {
                    std::string path = watch_map_[ev->wd];
                    if (ev->len > 0) path += "/" + std::string(ev->name);
                    std::lock_guard<std::mutex> l(mutex_);
                    events_.push_back({path, ev->mask, mono_ms()});
...
struct ScanResult {
    bool root, emulator, debugger, hook, tamper, integrity;
    bool suspicious_procs, suspicious_libs, mitm;
    bool verified_boot, encryption, tee;
    int  threat_count;
    std::string json;
};

static ScanResult full_comprehensive_scan(const char* pkg) {
    ScanResult r{};
    r.root             = detect_root();
    r.emulator         = detect_emulator();
    r.debugger         = detect_debugger();
    r.hook             = detect_hook();
    r.tamper           = detect_tamper();
    r.integrity        = detect_integrity(pkg);
    r.suspicious_procs = detect_suspicious_processes();
    r.suspicious_libs  = detect_suspicious_libs();
    r.mitm             = detect_mitm_proxy();
    r.verified_boot    = check_verified_boot();
    r.encryption       = check_encryption_status();
    r.tee              = check_tee_available();

    r.threat_count = (r.root?1:0)+(r.emulator?1:0)+(r.debugger?1:0)+(r.hook?1:0)+(r.tamper?1:0)+
                     (!r.integrity?1:0)+(r.suspicious_procs?1:0)+(r.suspicious_libs?1:0)+(r.mitm?1:0);
    g_threat_level.store(r.threat_count);

    std::ostringstream o;
    o << "{"
      << "\"root\":"             << (r.root ? "true":"false")             << ","
      << "\"emulator\":"         << (r.emulator ? "true":"false")         << ","
      << "\"debugger\":"         << (r.debugger ? "true":"false")         << ","
      << "\"hook\":"             << (r.hook ? "true":"false")             << ","
      << "\"tamper\":"           << (r.tamper ? "true":"false")           << ","
      << "\"integrity\":"        << (r.integrity ? "true":"false")        << ","
      << "\"suspicious_procs\":" << (r.suspicious_procs ? "true":"false") << ","
      << "\"suspicious_libs\":"  << (r.suspicious_libs ? "true":"false")  << ","
      << "\"mitm\":"             << (r.mitm ? "true":"false")             << ","
      << "\"verified_boot\":"    << (r.verified_boot ? "true":"false")    << ","
      << "\"encryption\":"       << (r.encryption ? "true":"false")       << ","
      << "\"tee\":"              << (r.tee ? "true":"false")              << ","
...
std::string full_comprehensive_scan_json(const char* pkg) {
    return full_comprehensive_scan(pkg).json;
}

extern "C" {

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckRoot(JNIEnv*, jobject) {
    return detect_root() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckEmulator(JNIEnv*, jobject) {
    return detect_emulator() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckDebugger(JNIEnv*, jobject) {
    return detect_debugger() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckHook(JNIEnv*, jobject) {
    return detect_hook() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckTamper(JNIEnv*, jobject) {
    return detect_tamper() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckIntegrity(JNIEnv* e, jobject, jstring jpkg) {
    auto pkg = j2s(e, jpkg);
    return detect_integrity(pkg.c_str()) ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT void JNICALL
Java_com_eaquel_service_GameAnticheat_nativeSetThreatLevel(JNIEnv*, jobject, jint lvl) {
    g_threat_level.store((int)lvl);
    LOGW("Tehdit seviyesi: %d", (int)lvl);
}

JNIEXPORT jint JNICALL
Java_com_eaquel_service_GameAnticheat_nativeGetThreatLevel(JNIEnv*, jobject) {
    return (jint)g_threat_level.load();
}

JNIEXPORT jstring JNICALL
Java_com_eaquel_service_GameAnticheat_nativeFullScan(JNIEnv* e, jobject) {
    bool root=detect_root(), emu=detect_emulator(), dbg=detect_debugger(),
         hook=detect_hook(), tamper=detect_tamper();
    int threats = (root?1:0)+(emu?1:0)+(dbg?1:0)+(hook?1:0)+(tamper?1:0);
    g_threat_level.store(threats);
    std::ostringstream o;
    o << "{"
      << "\"root\":"     << (root   ? "true":"false") << ","
      << "\"emulator\":" << (emu    ? "true":"false") << ","
      << "\"debugger\":" << (dbg    ? "true":"false") << ","
...
JNIEXPORT jstring JNICALL
Java_com_eaquel_service_GameAnticheat_nativeComprehensiveScan(JNIEnv* e, jobject, jstring jpkg) {
    auto pkg = j2s(e, jpkg);
    return s2j(e, full_comprehensive_scan_json(pkg.c_str()));
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeDetectSuspiciousProcs(JNIEnv*, jobject) {
    return detect_suspicious_processes() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeDetectSuspiciousLibs(JNIEnv*, jobject) {
    return detect_suspicious_libs() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeDetectMitm(JNIEnv*, jobject) {
    return detect_mitm_proxy() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckVerifiedBoot(JNIEnv*, jobject) {
    return check_verified_boot() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckEncryption(JNIEnv*, jobject) {
    return check_encryption_status() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckTee(JNIEnv*, jobject) {
    return check_tee_available() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckFridaGadget(JNIEnv*, jobject) {
    return detect_frida_gadget_in_memory() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jint JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCountLibraries(JNIEnv*, jobject) {
    return static_cast<jint>(count_mapped_libraries());
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckArtIntegrity(JNIEnv*, jobject) {
    return check_art_integrity() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckZygoteParent(JNIEnv*, jobject) {
    return check_zygote_parent() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeDetectVpn(JNIEnv*, jobject) {
    return detect_vpn() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeDetectAdbAlwaysOn(JNIEnv*, jobject) {
    return detect_adb_always_on() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckTimingAttack(JNIEnv*, jobject) {
    return detect_timing_attack() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeCheckVmAcceleration(JNIEnv*, jobject) {
    return detect_vm_acceleration_missing() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_com_eaquel_service_GameAnticheat_nativeProtectSelf(JNIEnv*, jobject) {
    return protect_self() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT void JNICALL
Java_com_eaquel_service_GameAdbBridge_nativeEnableSecureMode(JNIEnv*, jobject) {
    enable_secure_mode();
    LOGI("Güvenli mod etkinleştirildi");
}

JNIEXPORT jstring JNICALL
Java_com_eaquel_service_GameAdbBridge_nativeListProcesses(JNIEnv* e, jobject) {
    auto procs = scan_all_processes();
    std::ostringstream o;
    o << "[";
    bool first = true;
    for (auto& p : procs) {
        if (!first) o << ",";
        o << "{\"pid\":" << p.pid << ",\"name\":\"" << p.name << "\",\"uid\":" << p.uid << ",\"rss\":" << p.vmrss_kb << "}";
        first = false;
    }
    o << "]";
    return s2j(e, o.str());
...
JNIEXPORT jstring JNICALL
Java_com_eaquel_service_GameAdbBridge_nativeListMaps(JNIEnv* e, jobject) {
    auto regions = parse_maps();
    std::ostringstream o;
    o << "[";
    bool first = true;
...
JNIEXPORT void JNICALL
Java_com_eaquel_service_GameAdbBridge_nativeStartFsMonitor(JNIEnv* e, jobject, jobjectArray jpaths) {
    if (!e || !jpaths) return;
    std::vector<std::string> paths;
    jsize len = e->GetArrayLength(jpaths);
    for (jsize i = 0; i < len; ++i) {
        auto js = reinterpret_cast<jstring>(e->GetObjectArrayElement(jpaths, i));
        if (js) paths.push_back(j2s(e, js));
    }
    if (!g_fs_monitor) g_fs_monitor = std::make_unique<FileSystemMonitor>();
    g_fs_monitor->start(paths);
    LOGI("FS monitor başlatıldı: %zu yol", paths.size());
}

JNIEXPORT void JNICALL
Java_com_eaquel_service_GameAdbBridge_nativeStopFsMonitor(JNIEnv*, jobject) {
    if (g_fs_monitor) { g_fs_monitor->stop(); g_fs_monitor.reset(); }
    LOGI("FS monitor durduruldu");
}

JNIEXPORT jstring JNICALL
Java_com_eaquel_service_GameAdbBridge_nativeGetFsEvents(JNIEnv* e, jobject) {
    if (!g_fs_monitor) return s2j(e, "[]");
    auto events = g_fs_monitor->getEvents();
    std::ostringstream o;
    o << "[";
    bool first = true;
    for (auto& ev : events) {
        if (!first) o << ",";
        o << "{\"path\":\"" << ev.path << "\",\"mask\":" << ev.mask << ",\"ts\":" << ev.ts << "}";
        first = false;
    }
    o << "]";
    return s2j(e, o.str());
